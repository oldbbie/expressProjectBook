module.exports = {
	html:function(head,body){
		...
		`;
		return header;
	}, pageLink:function(countPage,showPageRange,nowPage){
		var i;
		...
		if(countPage > nextPage) {
			countPage = nextPage;
			var nextLink = `<li><a href="/episode/index/${nextPage+1}">다음</a></li>` ;
		}
		var listPage = '<ol>';
		if(nowPage>showPageRange){
			listPage += `<li><a class="checked" href="/episode/index/${provPage-1}">이전</a></li>` ;
			for(i = provPage; i <= countPage; i++) {
				if(i===nowPage) {
					listPage += `<li><a class="checked" href="/episode/index/${i}">${i}</a></li>` ;
				} else {
					listPage += `<li><a href="/episode/index/${i}">${i}</a></li>` ;
				}
			}
		} else {
