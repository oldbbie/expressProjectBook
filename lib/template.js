module.exports = {
	html:function(head,body){
		...
		return title;
	}, header:function(authStatusUI = '<a href="/account/">로그인</a>',selected){
		// 닉네임 선택창
		var header = `
			<header>
				<div class="left">
					...
					<h1><a href="/">소설이트</a></h1>
					<nav>`;
		header += selected==1 ? `<p><a href="/episode/" class="selected" >에피소드</a></p>` : `<p><a href="/episode/" >에피소드</a></p>`
		header += selected==2 ? `<p><a href="/book/" class="selected" >책</a></p>` : `<p><a href="/book/" >책</a></p>`
		header += selected==3 ? `<p><a href="/nickname/" class="selected" >작가</a></p>` : `<p><a href="/nickname/" >작가</a></p>`
		header += `
					</nav>
				</div>
				<div class="middle">
		...
		return listHotBook;
	}, selectBook:function(books,selected_id){
		var select = '<select name="book">';
		var i = 0;
		while(i < books.length){
			selected="";
			if(selected_id==books[i].id) {selected="selected";}
		  	select += `<option value=${books[i].id} ${selected}>${books[i].title}</option>`;
		  	i = i + 1;
		}
		select +='</select>';
		return select;
